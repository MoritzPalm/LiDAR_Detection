...
class VGGBase(nn.Module):
    ...
    def __init__(self):
        ...
        self.pool5 = nn.MaxPool2d(kernel_size=3, stride=1,
                                  padding=1)  # retains size because stride is 1
    ...
    def forward(self, image):
        ...
        out = self.pool3(
            out)  # (N, 256, 38, 38), it would have been 37 if not for ceil_mode = True
    ...
    def load_pretrained_layers(self):
        """
        As in the paper, we use a VGG-16 pretrained on the ImageNet task
        as the base network.
        ...
        # fc6
        conv_fc6_weight = pretrained_state_dict["classifier.0.weight"].view(4096, 512,
                                                                            7,
                                                                            7)
        # (4096, 512, 7, 7)
        conv_fc6_bias = pretrained_state_dict["classifier.0.bias"]  # (4096)
        state_dict["conv6.weight"] = decimate(conv_fc6_weight,
                                              m=[4, None, 3, 3])  # (1024, 512, 3, 3)
        state_dict["conv6.bias"] = decimate(conv_fc6_bias, m=[4])  # (1024)
        # fc7
        conv_fc7_weight = pretrained_state_dict["classifier.3.weight"].view(4096, 4096,
                                                                            1,
                                                                            1)
        # (4096, 4096, 1, 1)
        conv_fc7_bias = pretrained_state_dict["classifier.3.bias"]  # (4096)
        state_dict["conv7.weight"] = decimate(conv_fc7_weight, m=[4, 4, None,
                                                                  None])
        # (1024, 1024, 1, 1)
        state_dict["conv7.bias"] = decimate(conv_fc7_bias, m=[4])  # (1024)

        # Note: an FC layer of size (K) operating on a flattened version (C*H*W) of a
        # 2D image of size (C, H, W)...
        # ...is equivalent to a convolutional layer with
        # kernel size (H, W), input channels C, output channels K...
...
class AuxiliaryConvolutions(nn.Module):
    ...
    def forward(self, conv7_feats):
        ...
        a tensor of dimensions (N, 1024, 19, 19)
...
class PredictionConvolutions(nn.Module):
    """
    Convolutions to predict class scores and bounding boxes using lower
    and higher-level feature maps.

    The bounding boxes (locations) are predicted as encoded offsets w.r.t each
    of the 8732 prior (default) boxes.
    See 'cxcy_to_gcxgcy' in utils.py for the encoding definition.

    The class scores represent the scores of each object class in each
    of the 8732 bounding boxes located.
    ...
    def __init__(self, n_classes):
        ...
        self.loc_conv4_3 = nn.Conv2d(512, n_boxes["conv4_3"] * 4, kernel_size=3,
                                     padding=1)
        self.loc_conv7 = nn.Conv2d(1024, n_boxes["conv7"] * 4, kernel_size=3, padding=1)
        self.loc_conv8_2 = nn.Conv2d(512, n_boxes["conv8_2"] * 4, kernel_size=3,
                                     padding=1)
        self.loc_conv9_2 = nn.Conv2d(256, n_boxes["conv9_2"] * 4, kernel_size=3,
                                     padding=1)
        self.loc_conv10_2 = nn.Conv2d(256, n_boxes["conv10_2"] * 4, kernel_size=3,
                                      padding=1)
        self.loc_conv11_2 = nn.Conv2d(256, n_boxes["conv11_2"] * 4, kernel_size=3,
                                      padding=1)

        # Class prediction convolutions (predict classes in localization boxes)
        self.cl_conv4_3 = nn.Conv2d(512, n_boxes["conv4_3"] * n_classes, kernel_size=3,
                                    padding=1)
        self.cl_conv7 = nn.Conv2d(1024, n_boxes["conv7"] * n_classes, kernel_size=3,
                                  padding=1)
    ...
    def forward(self, conv4_3_feats, conv7_feats, conv8_2_feats, conv9_2_feats,
        ...
        :param conv4_3_feats: conv4_3 feature map, a tensor of dimensions
        (N, 512, 38, 38)
        :param conv7_feats: conv7 feature map, a tensor of dimensions (N, 1024, 19, 19)
        :param conv8_2_feats: conv8_2 feature map, a tensor of dimensions
        (N, 512, 10, 10)
        :param conv9_2_feats: conv9_2 feature map, a tensor of dimensions (N, 256, 5, 5)
        :param conv10_2_feats: conv10_2 feature map, a tensor of dimensions
        (N, 256, 3, 3)
        :param conv11_2_feats: conv11_2 feature map, a tensor of dimensions
        (N, 256, 1, 1)
        ...
        # (.contiguous() ensures it is stored in a contiguous chunk of memory,
        # needed for .view() below)
        l_conv4_3 = l_conv4_3.view(batch_size, -1,
                                   4)  # (N, 5776, 4),
        # there are a total 5776 boxes on this feature map
        ...
        c_conv8_2 = c_conv8_2.permute(0, 2, 3,
                                      1).contiguous()  # (N, 10, 10, 6 * n_classes)
        c_conv8_2 = c_conv8_2.view(batch_size, -1,
                                   self.n_classes)  # (N, 600, n_classes)

        c_conv9_2 = self.cl_conv9_2(conv9_2_feats)  # (N, 6 * n_classes, 5, 5)
        c_conv9_2 = c_conv9_2.permute(0, 2, 3,
                                      1).contiguous()  # (N, 5, 5, 6 * n_classes)
        c_conv9_2 = c_conv9_2.view(batch_size, -1,
                                   self.n_classes)  # (N, 150, n_classes)

        c_conv10_2 = self.cl_conv10_2(conv10_2_feats)  # (N, 4 * n_classes, 3, 3)
        c_conv10_2 = c_conv10_2.permute(0, 2, 3,
                                        1).contiguous()  # (N, 3, 3, 4 * n_classes)
        c_conv10_2 = c_conv10_2.view(batch_size, -1,
                                     self.n_classes)  # (N, 36, n_classes)

        c_conv11_2 = self.cl_conv11_2(conv11_2_feats)  # (N, 4 * n_classes, 1, 1)
        c_conv11_2 = c_conv11_2.permute(0, 2, 3,
                                        1).contiguous()  # (N, 1, 1, 4 * n_classes)
        c_conv11_2 = c_conv11_2.view(batch_size, -1,
                                     self.n_classes)  # (N, 4, n_classes)

        # A total of 8732 boxes
        # Concatenate in this specific order
        # (i.e. must match the order of the prior-boxes)
        locs = torch.cat(
...
class SSD300(nn.Module):
    """
    The SSD300 network - encapsulates the base VGG network,
    ...
    def __init__(self, n_classes):
        ...
        # Since lower level features (conv4_3_feats) have
        # considerably larger scales, we take the L2 norm and rescale
        # Rescale factor is initially set at 20,
        # but is learned for each channel during back-prop
    ...
    def forward(self, image):
        ...
        conv4_3_feats, conv7_feats = self.base(
            image)  # (N, 512, 38, 38), (N, 1024, 19, 19)
        ...
        conv8_2_feats, conv9_2_feats, conv10_2_feats, conv11_2_feats = \
            self.aux_convs(
                conv7_feats)  # (N, 512, 10, 10),  (N, 256, 5, 5),
        # (N, 256, 3, 3), (N, 256, 1, 1)

        # Run prediction convolutions
        # (predict offsets w.r.t prior-boxes and classes
        # in each resulting localization box)
            ...
            for i in range(fmap_dims[fmap]):
                for j in range(fmap_dims[fmap]):
                    ...
                    for ratio in aspect_ratios[fmap]:
                        ...
                        # For an aspect ratio of 1, use an
                        # additional prior whose scale is the geometric mean of the
                        # scale of the current feature map
    ...
    def detect_objects(self, predicted_locs, predicted_scores, min_score, max_overlap,
                       top_k):
        """
        Decipher the 8732 locations and class scores
        (output of ths SSD300) to detect objects.

        For each class, perform Non-Maximum Suppression (NMS)
        on boxes that are above a minimum threshold.

        :param predicted_locs: predicted locations/boxes
        w.r.t the 8732 prior boxes, a tensor of dimensions (N, 8732, 4)
        :param predicted_scores: class scores for each
        of the encoded locations/boxes, a tensor of dimensions (N, 8732, n_classes)
        :param min_score: minimum threshold
        for a box to be considered a match for a certain class
        :param max_overlap: maximum overlap two boxes can have so that
        the one with the lower score is not suppressed via NMS
        :param top_k: if there are a lot of resulting detection
        across all classes, keep only the top 'k'
        ...
        for i in range(batch_size):
            ...
            for c in range(1, self.n_classes):
                ...
                # where scores for this class are above the minimum score
                ...
                for box in range(class_decoded_locs.size(0)):
                    ...
                    # Suppress boxes whose overlaps (with this box)
                    # are greater than maximum overlap
                    # Find such boxes and update suppress indices
                    suppress = torch.max(suppress, overlap[box] > max_overlap)
                    # The max operation retains previously
                    # suppressed boxes, like an 'OR' operation

                    # Don't suppress this box, even though
                    # it has an overlap of 1 with itself
                    suppress[box] = 0
...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predicted_locs, predicted_scores, boxes, labels):
        ...
        :param predicted_locs: predicted locations/boxes w.r.t the 8732 prior boxes,
        ...
        for i in range(batch_size):
            ...
            # We don't want a situation where an object is not represented
            # in our positive (non-background) priors -
            # 1. An object might not be the best object for all priors,
            # and is therefore not in object_for_each_prior.
            # 2. All priors with the object may be assigned
            # as background based on the threshold (0.5).
            ...
            # Set priors whose overlaps with objects are less
            # than the threshold to be background (no object)
        ...
        # we will take the hardest (neg_pos_ratio * n_positives)
        # negative priors, i.e where there is maximum loss
        # This is called Hard Negative Mining -
        # it concentrates on hardest negatives in each image,
        ...
        conf_loss_neg[
            positive_priors] = 0.  # (N, 8732), positive priors are ignored
